...
class App:
    ...
    def create_camusb_source(self, pipeline, uri):
        source = Gst.ElementFactory.make("v4l2src", "usb-cam-source")
        source.set_property('device', uri)
        pipeline.add(source)

        caps_v4l2src = self.create_capsfilter("v4l2src_caps", "video/x-raw, framerate=30/1")
        ...
        caps_vidconvsrc = self.create_capsfilter("nvmm_caps", "video/x-raw(memory:NVMM), format=I420")
        ...
        caps_vidconvsrc.link(nvvidconv_1)
    ...
    def gst_pipeline(self, args):
        ...
        for i in range(number_sources):
            ...
            if uri_name.find("rtsp://") == 0 :
                ...
                srcpad.link(sinkpad)
            elif uri_name.find("usb:") == 0 :
                src_output = self.create_camusb_source(pipeline, uri_name.split('usb:')[1])
                padname="sink_%u" %i
                sinkpad = streammux.get_request_pad(padname)
                if not sinkpad:
                    sys.stderr.write(" Unable to get the sink pad of streammux \n")
                srcpad = src_output.get_static_pad("src")
                if not srcpad:
