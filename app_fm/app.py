    ...
    def osd_sink_pad_buffer_probe(self, pad,info,u_data):
        ...
        while l_frame is not None:
            ...
            while l_obj is not None:
                ...
                # Check violation
                violate = False
                has_request = self.track_manager.has_request_violate(track_id)
                label = str(track_id) + '_' + CLASS_PGIE[class_id]
                status = CLASS_PGIE[class_id]
                if class_id == 0:
                    box_color = (0,255,0)
                    violate = False
                ...
                if check_in_roi:
                    if (violate and not has_request):
                        ...
                        img_crop = np.array(org_frame, copy=True, order='C')
                        # convert the array into cv2 default color format
                        img_crop = cv2.cvtColor(img_crop, cv2.COLOR_RGBA2BGRA)

                        data_request = tracker.DataRequest(track_id, img_crop, status, violate, [x1, y1, x2, y2])
