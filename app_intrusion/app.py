...
class App:
    ...
    def is_obj_in_polygon(self, obj_meta, polygon):
        ...
        foot = is_point_in_polygon(polygon, (foot_x, foot_y))
    ...
    def osd_sink_pad_buffer_probe(self, pad,info,u_data):
        ...
        while l_frame is not None:
            ...
            # cv2.imwrite('frame.jpg', org_frame)
            ...
            while l_obj is not None:
                ...
                for idx, roi in enumerate(ROIs):
                    is_inside = self.is_obj_in_polygon(obj_meta, roi)
                    if is_inside:
                        violate = True
                        if not idx in roi_intrusion_idx:
                            roi_intrusion_idx.append(idx)
                ...
                # Check request
                need_warning, violate_age = self.track_manager.do_need_warning(track_id, violate=violate)
                label = str(track_id) + '_' + CLASS_PGIE[class_id] + '_' + str(violate_age)

                # TODO:
                # If:
                #   - check_in_roi == True
                #   - violate == True and track_id is not request yet 
                # Then: 
                #   - crop image
                ...
                if need_warning:
                    ...
                    x2 = int(x1 + rect_params.width)
                    y2 = int(y1 + rect_params.height)
                    data_request = tracker.DataRequest(track_id, org_frame, status, [x1,y1,x2,y2], True)
            ...
            for idx, roi in enumerate(ROIs):
                if not idx in roi_intrusion_idx:
                    n_frame_c = draw_polygon(n_frame_c, roi, (255,255,255))
                else:
                    n_frame_c = draw_polygon(n_frame_c, ROIs[idx], (255,0,0))
