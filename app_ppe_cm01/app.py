...
from utils import plot_box, draw_roi, is_in_roi, plot_box_grad, crop_object, is_inside_rect
import track_manager as tracker

OSD_PROCESS_MODE= 0
OSD_DISPLAY_TEXT= 1

CLASS_PGIE = ['Person', 'Person_Vest', 'Person_Hat', 'Person_Hat_Vest']
...
class App:
    def __init__(self) -> None:
        self.logo_ROI = [[COOR_X, COOR_Y], [536, 43]]
        h, w = self.logo_ROI[1][1] - self.logo_ROI[0][1], self.logo_ROI[1][0] - self.logo_ROI[0][0]  #43-5, 536 - 435
    ...
    def check_box_size_valid(self, obj_meta):
        rect_params = obj_meta.rect_params
        w = rect_params.width
        h = rect_params.height

        big_enough = False
        if w >= ROI[2]*0.5 and h >= ROI[3]*0.6:
            big_enough = True
        
        hw_ratio_valid = False
        if float(h/w) > 1.5:
            hw_ratio_valid = True

        if big_enough and hw_ratio_valid:
            return True
            
        return False

    def is_in_roi(self, obj_meta, ROI):
        rect_params = obj_meta.rect_params
        ...
        return is_inside_rect(ROI[0], ROI[1], ROI[0] + ROI[2], ROI[1] + ROI[3], center_x, center_y)
    ...
    def osd_sink_pad_buffer_probe(self, pad,info,u_data):
        ...
        while l_frame is not None:
            ...
            while l_obj is not None:
                ...
                check_in_roi = self.is_in_roi(obj_meta, ROI)
                check_box_size = self.check_box_size_valid(obj_meta=obj_meta)
                ...
                box_color = VIOLATE_COLOR

                # Check violation
                violate = False
                has_request, live_ages = self.track_manager.has_request_violate(track_id)
                label = str(track_id) + '_' + CLASS_PGIE[class_id] + '_' + str(live_ages)
                # label = CLASS_PGIE[class_id]
                if class_id == 0:
                    status = 'Miss PPE items' # hat, vest'
                    violate = True
                if class_id == 1:
                    status = 'Miss PPE items' #vest'
                    violate = True
                if class_id == 2:
                    status = 'Miss PPE items' #hat'
                    violate = True
                if class_id == 3:
                    status = ''
                    box_color = UNVIOLATE_COLOR
                    violate = False
                ...
                #   - check box size
                #   - violate == True
                #   - track_id is not request yet
                #   - enough live_ages  
                # Do: 
                #   - crop image
                #   - create data request
                #   - add data request to queue
                if check_in_roi and check_box_size:
                    if (violate and not has_request and live_ages > LIVE_AGES_MIN):
                        ...
                        data_request = tracker.DataRequest(track_id, img_crop, status, violate)
                ...
                ## Draw frame
                if check_in_roi and check_box_size:
                    n_frame_c = plot_box_grad(n_frame_c, obj_meta, box_color, label)
            ...
            n_frame_c = draw_roi(n_frame_c, ROI)
    ...
    def gst_pipeline(self, args):
        ...
        streammux.set_property('width', STREAM_W)
        streammux.set_property('height', STREAM_H)
        ...
        videocrop.set_property('left', VIDEO_CROP_L)
        videocrop.set_property('right', VIDEO_CROP_R)
        videocrop.set_property('top', VIDEO_CROP_T)
        videocrop.set_property('bottom', VIDEO_CROP_B)
        ...
        caps_w = ROI[2]
        caps_h = ROI[3]
        cap_str = "video/x-raw(memory:NVMM),width="+ str(caps_w) + ",height=" + str(caps_h)
        caps_crop.set_property('caps', Gst.Caps.from_string(cap_str))
        ...
        sink.set_property("overlay-x", SINK_OVERLAY_x)
        sink.set_property("overlay-y", SINK_OVERLAY_y)
        sink.set_property("overlay-w", SINK_OVERLAY_w)
        sink.set_property("overlay-h", SINK_OVERLAY_h)
